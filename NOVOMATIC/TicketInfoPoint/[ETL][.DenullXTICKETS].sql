...
*/
ALTER FUNCTION [ETL][.DenullXTICKETS](@XTICKETS XML)
RETURNS XML
AS
BEGIN
	DECLARE	@returnXML XML
	SET		@returnXML = (SELECT * FROM	ETL.GetAllXTICKETS(@XTICKETS) FOR XML RAW('TICKETS'), ELEMENTS XSINIL)
	...
	SET	@returnXML = (
		...
		FROM	@returnXML.nodes('//TICKETS') AS T(c)
